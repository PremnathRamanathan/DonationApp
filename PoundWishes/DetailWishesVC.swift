...
class DetailWishesVC: UIViewController {
    ...
    // FB Share configuration
    func shareOnFB(){
        let contentDescription = "Make your donation now"
        let url = NSURL(string: "https://www.PoundWishes.com")
        let contentTitle = NameLabel.text
        let content = FBSDKShareLinkContent()
        content.contentTitle = contentTitle
        content.contentDescription = contentDescription
        content.contentURL = url! as URL!
        
        let shareButton = FBSDKShareButton()
        shareButton.shareContent = content
        shareButton.frame =  CGRect(x: 260, y: 160, width: 100, height: 30)
